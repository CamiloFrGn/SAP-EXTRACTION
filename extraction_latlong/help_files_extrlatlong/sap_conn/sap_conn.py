...
def run_sap_gui(objSess,transaccion,variante,fecha,path,filename):
    ...
    try:
        ...
        objSess.findById("wnd[1]/usr/txtV-LOW").caretPosition = 11
        objSess.findById("wnd[1]/tbar[0]/btn[8]").press()
        objSess.findById("wnd[0]/usr/ctxtS_DATE-LOW").Text = fecha
        objSess.findById("wnd[0]/usr/ctxtP_LAYOUT").Text = "/LATLONGV2"
        objSess.findById("wnd[0]/usr/ctxtP_LAYOUT").SetFocus()
        objSess.findById("wnd[0]/usr/ctxtP_LAYOUT").caretPosition = 6
