...
def send_df_to_sql(data,database_name):
    try:    
        ...
        engine = sa.create_engine("mssql+pyodbc:///?odbc_connect=%s" % params, fast_executemany=True, poolclass=NullPool)
        conn = engine.connect()
        ...
        size_in_mb = (final_size) / (1024**2)

        print(f"Data transfer size: "+str(size_in_mb)+" MB")
        return "success"       
...
def query_sql_crud(query, parameters,data,database_name):   
    try:     
...
def query_sql_crud_select(query, parameters):   
    try:     
        #reference the odbc module to open connection 
        connection = pyodbc.connect(connection_string)
        #declare cursor for sql query execution
